...
// const clickButtons = require('../../../audio/click-button.mp3');

// const audioButton = new Audio(clickButtons.default);
...
const Game = (props: any): JSX.Element => {
	...
	useEffect(() => {
		...
		return () => {
			document.removeEventListener('keydown', buttonKey);
		};
	});
	...
	const getPlayers = props.players.map((players: Rec) => (
		<PlayersGame
			key={players.id}
			id={players.id}
			profile={players.image}
			name={players.name}
			images={props.images}
	...
	return (
		<div>
			<div className='optional-volume'>
				{marks ? (
					<div className='optional-volume__mark'>
						<h3>Instruction</h3>
						...
						<button onClick={() => setMarks((!marks))}>close</button>
					</div>
				) : (
					<img
						onFocus={() => { console.log("focus") }}
						alt='img'
						style={{ marginRight: '20px', borderRadius: '90px' }}
						src={mark}
						onMouseOver={() => setMarks(true)}
					/>
				)}
				<img
					alt='img'
				...
				{setting ? (
					<div className='setting'>
						<div style={{ marginTop: '20px' }}>
							<strong>Music :</strong>{' '}
							<button onClick={musicAudio}>On/Off</button>
							<br />
						</div>
						<div style={{ marginTop: '20px' }}>
							<strong>click :</strong>{' '}
							<button onClick={clickAudio}>On/Off</button>
						</div>
						<div style={{ marginTop: '20px' }}>
							<strong>Volume music:</strong>
							<input
								onChange={(e) => {
						...
						<div style={{ marginTop: '10px' }}>
							...
							<input
								type='color'
								value={colorQuestion}
								onChange={(e) => setColorQuestion(e.target.value)}
							/>
						</div>
					</div>
				) : null}
			...
			<div className='start-grid'>
				<div
					...
					className='start-grid__players'>
					{getPlayers}
				</div>
				<div
					className='start-grid__questions'
					style={{ background: `${colorQuestion}` }}>
					<Questions
				...
				<div className='start-grid__stats-players'>
					<StatsPlayers
						theme={props.theme}
						question={props.question}
						getTheme={props.getTheme}
